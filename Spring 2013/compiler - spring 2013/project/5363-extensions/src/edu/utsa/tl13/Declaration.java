...
public class Declaration extends GeneralStatement {
	...
	public void addToSymbolTable(String identifier, String type){
		...
		if (st.getType(identifier) == null){
			if (ident.exp != null){ // variable is an array.
				int arraysize = (int) ident.exp.l_s_exp.l_term.l_factor.numNode.value;
				if (arraysize> 0) {
					MIPSWriter mw = MIPSWriter.getInstance("already_created.s"	);
					mw.addArrayVariable(identifier, arraysize);
					SymbolTable.getInstance().addToSymbolTable(identifier, type,true);
