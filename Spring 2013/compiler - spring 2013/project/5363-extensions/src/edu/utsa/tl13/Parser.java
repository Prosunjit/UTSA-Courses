...
public class Parser {
	...
	public void parse(){
		...
		DeclarationList dl;
		String [] words;
		ArrayList<String> tl13words;
		
		String fileContent = FileOperation.fileContent(InputFileName + ".tl13");
		LexicalAnalyzer la = new LexicalAnalyzer(fileContent);
		tl13words  =  la.getTokens();
		debug(""+tl13words);

		
		Tokenizer tokenizer = new Tokenizer(tl13words);		
		tokens= tokenizer.getTokens();
		try {
			MIPSWriter mw = MIPSWriter.getInstance(InputFileName+".s");
			if ( (p = program())!=null) {
			p.draw(0);
	...
	public DeclarationList declarations( ){
		...
		{ 
			DeclarationList.getInstance().AddDeclaration(new Declaration(ident, tn));
			return DeclarationList.getInstance();
		}
		else return null;
	}
	...
	public GeneralStatement statement( ) {
		...
		else if (seek_next_word().equals("EXIT")){
			return exit();
		}
		else Error_Class.showMessage("Syntax Error  in statement");
		return null;
	}
	public GeneralStatement exit(){
		if (fetch_word("EXIT")){
	...
	public GeneralStatement assignment( ){
		...
		if ( 
			...
			ident.type = SymbolTable.getInstance().getType(ident.name);
			return new Assignment(ident, new OpNode(assgn,Constant.ASSGN), b1);
		}
		else {
	...
	public Beta2 beta2(){
		...
		if (
				...
				seek_next_word().equals("ArrayEnd")
				) return new Beta2();
		else if ( 
	...
	public Beta4 beta4(){
		...
		Factor f;
		if (
			Arrays.asList("OP3","OP4","SC","DO","THEN","RP","ArrayEnd").contains(seek_next_word())
			) return new Beta4();
		else if (
	...
	public Factor factor(){
		...
		//else if ( seek_next_word().equals("ident") && !(identName=fetch("ident")).equals(""))  {
		 else if ( seek_next_word().equals("ident") )  {
	...
	public IdentNode ident(){
		String identName = fetch("ident");
		if (seek_next_word().equals("ArrayStart")){
			fetch("ArrayStart");
			Expression exp = expression();
			fetch("ArrayEnd");
			return new IdentNode(identName,Constant.ARRAY,exp);
		}
		return new IdentNode(identName,"ident");
	}
