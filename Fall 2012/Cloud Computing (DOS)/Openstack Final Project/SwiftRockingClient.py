...
def lost_connection():
  print "Ups... connection is lost... :( You have to login again"
  menu()
...
def upload_large_file():
  ...
  try:
      ...
      modified_swift.parser_prep(['modified_swift.py', '-A', SERVER, '-U', TENANT_USER, '-K', USER_PASS, 'upload', CONTAINER, FILE, '-S', str(SIZE)])
      modified_swift.call_parser(['upload', CONTAINER, FILE, '-S', str(SIZE)],['modified_swift.py', '-A', SERVER, '-U', TENANT_USER, '-K', USER_PASS, 'upload', CONTAINER, FILE, '-S', str(SIZE)])
      b = datetime.datetime.now()
      c = b - a
      print "Successfully uploaded... ;) " 
      print "**** total required time ",c.seconds , " seconds ****"
  except Exception as e:
    print "Something bad happened. ", e
  
  menu()

def determine_best_segmentation():
  global connection
  global USER_PASS
  global TENANT_USER
  global FILE
  ...
  FILE = path + filename
  CONTAINER = raw_input("Name of the Container on the server: ")
  print " !! This program now find best segmentation strategy considering (64, 128, 256 or no segmentation)"
  print " !! This will take a while. If you like, in the meantime,  you can go & have a coffee :p "
  #SEGMENT = raw_input("Enter segment size (MB): ")
  SIZE = 64 * pow(2,20)
  #debug(SIZE )
  if not connection:
  ...
  try:
      ...
      b = datetime.datetime.now()
      c = b - a
      print "**** total required time with 64MB segment is  ",c.seconds , " seconds ****"
      modified_swift.parser_prep(['modified_swift.py', '-A', SERVER, '-U', TENANT_USER, '-K', USER_PASS, 'delete', CONTAINER, FILE])
      modified_swift.call_parser(['delete', CONTAINER, FILE] ,['modified_swift.py', '-A', SERVER, '-U', TENANT_USER, '-K', USER_PASS, 'delete', CONTAINER, FILE])
      ...
      b = datetime.datetime.now()
      c = b - a
      print "**** total required time with 128 MB segment is  ",c.seconds , " seconds **** "
      modified_swift.parser_prep(['modified_swift.py', '-A', SERVER, '-U', TENANT_USER, '-K', USER_PASS, 'delete', CONTAINER, FILE])
      modified_swift.call_parser(['delete', CONTAINER, FILE] ,['modified_swift.py', '-A', SERVER, '-U', TENANT_USER, '-K', USER_PASS, 'delete', CONTAINER, FILE])
      ...
      b = datetime.datetime.now()
      c = b - a
      print "**** total required time with 256 MB segment is  ",c.seconds , " seconds ****"
      modified_swift.parser_prep(['modified_swift.py', '-A', SERVER, '-U', TENANT_USER, '-K', USER_PASS, 'delete', CONTAINER, FILE])
      modified_swift.call_parser(['delete', CONTAINER, FILE] ,['modified_swift.py', '-A', SERVER, '-U', TENANT_USER, '-K', USER_PASS, 'delete', CONTAINER, FILE])
      ...
      a = datetime.datetime.now()
      modified_swift.parser_prep(['modified_swift.py', '-A', SERVER, '-U', TENANT_USER, '-K', USER_PASS, 'upload', CONTAINER, FILE])
      modified_swift.call_parser(['upload', CONTAINER, FILE, '-S', str(SIZE)],['modified_swift.py', '-A', SERVER, '-U', TENANT_USER, '-K', USER_PASS, 'upload', CONTAINER, FILE])
      b = datetime.datetime.now()
      c = b - a
      print "**** total required time with no  segment is  ",c.seconds , " seconds **** "
      modified_swift.parser_prep(['modified_swift.py', '-A', SERVER, '-U', TENANT_USER, '-K', USER_PASS, 'delete', CONTAINER, FILE])
      modified_swift.call_parser(['delete', CONTAINER, FILE] ,['modified_swift.py', '-A', SERVER, '-U', TENANT_USER, '-K', USER_PASS, 'delete', CONTAINER, FILE])
  except Exception as e:
    print "Something bad happened. ", e
...
def menu():
  actions={'l':'login Regular','L':'Admin Login','u':'Upload Small Object','U':'Upload Large object','I':'lIst Objects','q':'Quit','c':'Create Container','d':'Delete Object','w':'Download file','b':'Best Segmentation finding'}
  print "          ** Swift Client Menu**         ","\n"
