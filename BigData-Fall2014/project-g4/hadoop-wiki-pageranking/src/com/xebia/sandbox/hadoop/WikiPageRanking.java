...
public class WikiPageRanking extends Configured implements Tool {
	...
	System.out.println(args) ;
        boolean isCompleted =  runXmlParsing(inputFile,outputFile + "/iter00");
	//boolean isCompleted = runXmlParsing("wiki/soc", "wiki/soc-out/iter00");
        if (!isCompleted) return 1;

        String lastResultPath = null;

        for (int runs = 0; runs < iteration; runs++) {
            String inPath = outputFile + "/iter" + nf.format(runs);
            lastResultPath = outputFile + "/iter" + nf.format(runs + 1);
        ...
        isCompleted = runRankOrdering(lastResultPath, args[3]);
